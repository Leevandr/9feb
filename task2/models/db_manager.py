"""Модуль работы с базой данных мебельного магазина."""
import sqlite3
import os
...
def load_products():
    ...
    rows = conn.execute('''
        SELECT p.*, c.Name AS CategoryName,
               m.Name AS ManufacturerName, mt.Name AS MaterialName
        FROM Products p
        JOIN Categories c ON p.CategoryID = c.CategoryID
        JOIN Manufacturers m ON p.ManufacturerID = m.ManufacturerID
        JOIN Materials mt ON p.MaterialID = mt.MaterialID
    ''').fetchall()
    conn.close()
...
def add_product(data):
    ...
    conn.execute('''
        INSERT INTO Products
            (Name, CategoryID, Description, ManufacturerID, MaterialID,
             Price, Dimensions, StockQuantity, Discount, ImagePath)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        data['Name'], data['CategoryID'], data['Description'],
        data['ManufacturerID'], data['MaterialID'], data['Price'],
        data['Dimensions'], data['StockQuantity'], data['Discount'],
        data['ImagePath']
    ))
    conn.commit()
    conn.close()
...
def update_product(product_id, data):
    ...
    conn.execute('''
        UPDATE Products SET
            ...
            ManufacturerID = ?, MaterialID = ?, Price = ?,
            Dimensions = ?, StockQuantity = ?, Discount = ?,
            ImagePath = ?
        WHERE ProductID = ?
    ''', (
        data['Name'], data['CategoryID'], data['Description'],
...
def load_materials():
    conn = get_connection()
    rows = conn.execute(
        'SELECT * FROM Materials ORDER BY Name').fetchall()
    conn.close()
    return [dict(r) for r in rows]
