"""Форма добавления / редактирования товара (мебель)."""
import os
...
from models.db_manager import (
    load_categories, load_manufacturers, load_materials,
...
class ProductForm(QWidget):
    def __init__(self, parent_window, product=None):
        ...
        for mt in self.materials:
            self.ui.material_combo.addItem(
                mt['Name'], mt['MaterialID'])
    ...
    def _fill_from_product(self):
        ...
        idx = self.ui.material_combo.findData(p['MaterialID'])
        if idx >= 0:
            self.ui.material_combo.setCurrentIndex(idx)

        self.ui.price_spin.setValue(p['Price'])
        self.ui.dimensions_edit.setText(p.get('Dimensions', '') or '')
    ...
    def save(self):
        ...
        data = {
            ...
            'MaterialID': self.ui.material_combo.currentData(),
            'Price': self.ui.price_spin.value(),
            'Dimensions': self.ui.dimensions_edit.text().strip(),
